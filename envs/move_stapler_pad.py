...
class move_stapler_pad(Base_Task):
    ...
    def _get_observer_focus_point(
        ...
        return np.asarray(self.stapler.get_pose().p, dtype=np.float64)
    ...
    def play_once(self):
        ...
        operator_grasp_seq = self.grasp_actor(self.stapler, arm_tag=arm_tag, pre_grasp_dis=0.1)
        self._move_with_observer_tracking(operator_grasp_seq, arm_tag)
        # Move the arm upward by 0.1 meters along z-axis
        operator_lift_seq = self.move_by_displacement(arm_tag, z=0.1, move_axis="arm")
